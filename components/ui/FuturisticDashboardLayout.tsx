...
export default function FuturisticDashboardLayout({
  children,
}: {
  ...
  useEffect(() => {
    ...
    // Update active path on mount and route changes
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-gray-950 via-black to-gray-900 min-w-0">
      {/* Sidebar */}
      <aside
        className={`${
          sidebarOpen ? "w-64" : "w-20"
        } bg-black/60 backdrop-blur-xl border-r border-gray-800/50 transition-all duration-300 hidden md:flex md:flex-col relative overflow-hidden`}
        style={{
          boxShadow: 'inset -1px 0 0 0 rgba(0, 255, 136, 0.1)'
        }}
      >
        {/* Effet de lueur sur le bord */}
        <div className="absolute top-0 right-0 w-px h-full bg-gradient-to-b from-transparent via-[#00ff88]/30 to-transparent" />
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="relative w-64 h-full bg-black/90 backdrop-blur-xl border-r border-gray-800/50">
            <SidebarInner />
          </div>
        </div>
      )}
      ...
      <main className="flex-1 overflow-y-auto min-w-0 pt-0 md:pt-0">
        <div className="md:hidden p-2 border-b border-gray-800/50 bg-black/60 backdrop-blur-xl"> 
          <Button 
            isIconOnly 
            variant="light" 
            size="sm" 
          ...
          >
            <Icon icon="mdi:menu" className="text-xl text-[#00ff88]" />
          </Button>
        </div>
        {children}
      </main>
